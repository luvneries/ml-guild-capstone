...
USER_ITEM_DATA_FILE = 'data/u.data'
...
class Recommendations(object):
    ...
    def _load_model(self, local_model_path):
        ...
        # load npy arrays for user/item factors and user/item maps
        self.user_factor = np.load(ROW_MODEL_FILE)
        self.item_factor = np.load(COL_MODEL_FILE)
        self.user_map = np.load(USER_MODEL_FILE)
        self.item_map = np.load(ITEM_MODEL_FILE)

        logging.info('Finished loading arrays.')
        # load user_item history into pandas dataframe
        ratings_df = pd.read_csv(USER_ITEM_DATA_FILE,
                                 sep='\t',
                                 names=['user_id', 'item_id', 'ratings', 'timestamp'])
        self.user_items = ratings_df.groupby('user_id')
    ...
    def get_recommendations(self, user_id, num_recs):
        ...
        if user_idx:
            ...
            already_rated = self.user_items.get_group(user_id).item_id
        ...
        if user_id:
            # get already viewed items from views dataframe
            already_rated = self.user_items.get_group(user_id).item_id.values

            # generate list of recommended article indexes from model
            item_recommendations = generate_recommendations(user_id, already_rated,
                                                       self.user_factor,
                                                       self.item_factor,
                                                       num_recs)
